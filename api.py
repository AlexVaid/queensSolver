...
def solve():
    ...
    # Декодуємо зображення з base64
    image_bytes = base64.b64decode(image_data)
    image = Image.open(io.BytesIO(image_bytes))
    image_np = np.array(image)
    image_bgr = cv2.cvtColor(image_np, cv2.COLOR_RGB2BGR)

    # Обробка зображення та розв'язання задачі
    image_processor = ImageProcessor(image_bgr, n)
    board = ChessBoard(n, image_processor)
    strategy = BacktrackingSolver()
    solver = QueensSolver(board, strategy)
    solver.solve()

    # Візуалізація дошки
    BoardRenderer.render_board(board, highlight_conflicts=True)
    plt.savefig('solution.png')
    plt.close()

    # Зчитуємо зображення рішення та кодуємо його в base64
    with open('solution.png', 'rb') as f:
        img_bytes = f.read()
    encoded_img = base64.b64encode(img_bytes).decode('utf-8')

    return jsonify({'solution_image': encoded_img})
